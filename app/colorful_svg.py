...
def color_quantization(path, output_dir, num_colors, save_png: bool = True, add_label: bool = False):
    dir_path = output_dir
...
def update_svg_colors(svg_path, updates):
    ...
    temp_dir = resolve_relative_path(__file__, '../temp/edit_svg')
    os.makedirs(temp_dir, exist_ok=True)

    new_file_path = f'{temp_dir}/edit_{name}.svg'
...
def create_color_svg(quantized_image, colors, width, height, output_path, add_label):
    temp_dir = resolve_relative_path(__file__, '../temp/temp_potrace')
    os.makedirs(temp_dir, exist_ok=True)
